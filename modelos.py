...
class Usuarios(db.Model):
    ...
    created_at = db.Colum(db.DateTime, default=datetime.datetime.now())
...
class Destinos(db.Model):
    ...
    created_at = db.Colum(db.DateTime, default='2024-06-16 00:0:00')
...
class Resenias(db.Model):
    ...
    paquete_id = db.Column(db.Integer, db.ForeignKey('paquetes_turiticos.id'))
    user_id = db.Column(db.Integer, db.ForeignKey('usuarios.id'))
    
class Reservas(db.Model):
    __tablename__ = 'reservas'
    id =db.Column(db.Integer, primary_key=True, nullable=False)
    created_at = db.Colum(db.DateTime, default='2024-07-01 00:0:00')
